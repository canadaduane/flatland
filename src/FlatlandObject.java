...
class FlatlandObject {
    ...
    static final double FRICTION_COEFFICIENT = 0.1;
    FlatlandShape shape;
...
class Eye extends FlatlandObject {
    ...
    public FlatlandPoint firstVisible(double lookDirection) {
        FlatlandPoint outerPerimeter = new FlatlandPoint(
        ...
        for (int i = 0; i < listOfObjects.length; i++) {
            ...
            if (flObj != this && flObj != objectOfAttachment) {
                ...
                int k = 1;
                for (int j = 0; j < flObj.shape.tail; j++) {
                    endPoint = shape.center.intersect(
                        outerPerimeter,
                        flObj.shape.point[j],
                        flObj.shape.point[k]
                    );
            ...
            for (double d = -Math.PI / 4; d <= Math.PI / 4; d += Math.PI / 2 / TRACE_LINES) {
                ...
                if (point != null) {
                    ...
                    if (showRays) {
                        ...
                        g.drawLine(
                            (int) shape.center.xFromOrigin(),
                            -(int) shape.center.yFromOrigin(),
                            (int) point.xFromOrigin(),
                            -(int) point.yFromOrigin()
                        );
                        g.setColor(Color.RED);
                        g.fillOval(
                            (int) point.xFromOrigin() - 2,
                            -(int) point.yFromOrigin() - 2,
                            5,
                            5
                        );
                    ...
                    if (getCanvas().useColor) {
                        ...
                        visorGraphics.setColor(
                            new Color(
                                (intensity * red) / 255,
                                (intensity * green2) / 255,
                                (intensity * blue) / 255
                            )
                        );
                    } else {
